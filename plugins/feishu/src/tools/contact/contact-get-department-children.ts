...
import {
  parseContactGetDepartmentChildrenBody,
  parseContactGetDepartmentChildrenQuery,
} from "./contact-get-department-children.zod"

import contact_get_department_childrenSkill from "./contact-get-department-children-skill.md" with {
  type: "text",
}
...
export const feishuContactGetDepartmentChildrenTool: ToolDefinition = {
  ...
  invoke: async ({ args }) => {
    ...
    const query = parseContactGetDepartmentChildrenQuery(queryRaw)
    const body = parseContactGetDepartmentChildrenBody({})
