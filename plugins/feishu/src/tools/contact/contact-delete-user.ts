...
import {
  parseContactDeleteUserBody,
  parseContactDeleteUserQuery,
} from "./contact-delete-user.zod"

import contact_delete_userSkill from "./contact-delete-user-skill.md" with {
  type: "text",
}
...
export const feishuContactDeleteUserTool: ToolDefinition = {
  name: `feishu-${fn.id}`,
  display_name: {
  ...
  invoke: async ({ args }) => {
    ...
    const queryParams = parseContactDeleteUserQuery(queryRaw) as Record<
      string,
      unknown
    >
    const body = parseContactDeleteUserBody(bodyRaw) as Record<string, unknown>
