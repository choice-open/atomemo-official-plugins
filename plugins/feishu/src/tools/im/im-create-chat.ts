...
import {
  parseImCreateChatBody,
  parseImCreateChatQuery,
} from "./zod/im-create-chat.zod"

import im_create_chatSkill from "./im-create-chat-skill.md" with {
  type: "text",
}
...
export const feishuImCreateChatTool: ToolDefinition = {
  ...
  invoke: async ({ args }) => {
    ...
    const query = parseImCreateChatQuery(queryRaw)
    const body = parseImCreateChatBody(bodyRaw)
