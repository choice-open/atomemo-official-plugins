...
export const getFollowersTool = {
  name: "twitter-get-followers",
  display_name: t("TWITTER_TOOL_GET_FOLLOWERS_DISPLAY_NAME"),
  description: t("TWITTER_TOOL_GET_FOLLOWERS_DESCRIPTION"),
  icon: "👥",
  skill: getSkill("get-followers"),
  parameters: [
    twitterCredentialParam,
    userIdParam,
    maxResultsParam,
  ...
  async invoke({ args }) {
    ...
    const res = await client.users.getFollowers(user_id, {
      userFields: [
        "created_at",
        "description",
        "public_metrics",
