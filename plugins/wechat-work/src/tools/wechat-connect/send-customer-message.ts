...
const sendCustomerMessageBodySchema = z.object({
  chat_type: z
    .enum(["single", "group"])
    .optional()
  ...
  tag_filter: z
    ...
    .optional()
    .describe("客户标签筛选条件，同组标签之间按或关系筛选，不同组标签按且关系筛选"),
  sender: z
    .string()
    .optional()
...
• chat_type (optional, string): "single" (to customers, default) or "group" (to customer groups)
• external_userid (optional, string[]): customer externaluserid list, max 10000, only for single
• chat_id_list (optional, string[]): customer group id list, max 2000, only for group
• tag_filter (optional, obj): { group_list: [{ tag_list: string[] }] }
• sender (optional, string): member userid, required when chat_type is group
• allow_select (optional, bool): allow reselection, default false, only for single
• text (optional, obj): { content: string }, max 4000 bytes
• attachments (optional, obj[]): max 9, each with msgtype + type-specific fields`,
...
• attachments（选填，obj[]）：附件，最多9个，每个包含 msgtype 和对应类型的字段（image/link/miniprogram/video/file）
注意：text和attachments不能同时为空`,
  ...
  async invoke({ args }) {
    ...
    const parsed = sendCustomerMessageBodySchema.safeParse(rawBody)
    if (!parsed.success) {
