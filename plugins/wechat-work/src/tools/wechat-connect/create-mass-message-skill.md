# 创建企业群发 (`wechat-work-create-mass-message`)
...
| tag_filter | 否 | obj | 标签筛选 { group_list: [{ tag_list: string[] }] }，同组按或关系，不同组按且关系 |
...
| attachments | 否 | obj[] | 附件，最多支持添加9个附件，支持 image/link/miniprogram/video/file |

注意：text和attachments不能同时为空。调用该接口并不会直接发送消息给客户/客户群，需要成员确认后才会执行发送。
