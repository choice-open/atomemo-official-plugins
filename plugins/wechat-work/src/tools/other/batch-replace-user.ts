...
import batchReplaceUserSkill from "./batch-replace-user-skill.md" with { type: "text" }
...
export const batchReplaceUserTool: ToolDefinition = {
  name: "wechat-work-batch-replace-user",
  display_name: {
    en_US: "Batch replace members (full)",
    zh_Hans: "全量覆盖成员",
  },
  description: {
    en_US: "Fully replace all members in WeChat Work organization.",
    zh_Hans: "全量覆盖企业微信通讯录成员。",
  },
  skill: batchReplaceUserSkill,
  icon: "🔁",
  parameters: [
    {
    ...
    },
    {
      ...
      ui: {
        component: "input",
        hint: {
          en_US: "Sync ID for incremental sync mode",
          zh_Hans: "增量同步模式下的同步ID",
        },
        support_expression: true,
        width: "full",
      },
  ...
  async invoke({ args }) {
    ...
    const syncId = params.sync_id?.trim()
    if (syncId) {
      body.sync_id = syncId
    }

    const data = await wechatWorkPostJson<BatchReplaceUserResponse>(
      "/batch/replaceuser",
      token,
      body,
    )
