...
const settingsSchema = z.object({
  ...
  remind_scope: z
    .number()
    .optional()
    .describe("来电提醒方式。1-不提醒；2-仅主持人；3-所有成员；4-指定部分人。默认提醒所有成员"),
  enable_enter_mute: z
    .number()
    .optional()
...
const remindersSchema = z.object({
  ...
  repeat_type: z
    .number()
    .optional()
    .describe("周期性会议重复类型。0-每天；1-每周；2-每月；7-工作日。默认0"),
  repeat_until: z
    .number()
    .optional()
    .describe("重复结束时刻。每天/工作日/每周最多200次，每两周/每月最多50次"),
  repeat_interval: z
    .number()
    .optional()
    .describe("重复间隔，目前仅repeat_type=1（每周）时支持，值不能大于2"),
  remind_before: z
    .array(z.number())
    .optional()
...
const updateMeetingBodySchema = z.object({
  ...
  meeting_start: z
    .number()
    .optional()
    .describe("会议开始时间的Unix时间戳，需大于当前时间。修改时须同时指定meeting_duration"),
  meeting_duration: z
    .number()
    .optional()
    .describe("会议持续时间（秒），最小300秒，最大86399秒。修改时须同时指定meeting_start"),
  description: z
    .string()
    .optional()
...
• remind_time（选填，uint32）：会议开始前多久提醒成员（秒），默认0
