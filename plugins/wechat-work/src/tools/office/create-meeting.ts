...
const settingsSchema = z.object({
  ...
  enable_enter_mute: z
    ...
    .optional()
    .describe(
      "成员入会时静音。1-开启；0-关闭；2-超过6人自动开启。默认超过6人自动开启",
    ),
...
const createMeetingBodySchema = z.object({
  ...
  title: z.string().max(40).describe("会议标题，最多40字节或20个utf8字符"),
  meeting_start: z
    .number()
...
• title (required, string): title, max 40 bytes or 20 utf8 chars
• meeting_start (required, uint32): start time, Unix timestamp, must > now
• meeting_duration (required, uint32): duration in seconds, 300-86399
...
• title（必填，string）：会议标题，最多40字节或20个utf8字符
• meeting_start（必填，uint32）：会议开始时间的Unix时间戳，需大于当前时间
• meeting_duration（必填，uint32）：会议持续时间（秒），最小300秒，最大86399秒
