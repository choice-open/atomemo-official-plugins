...
> 参考来源：[https://developers.google.com/workspace/gmail](https://developers.google.com/workspace/gmail)
...
- **发现文档**：[https://gmail.googleapis.com/$discovery/rest?version=v1](https://gmail.googleapis.com/$discovery/rest?version=v1)
...
- ✅ 创建、更新、删除标签（不含 patch）
- ✅ 管理线程（获取、列表、修改标签、移至垃圾箱；不含永久删除）
- ✅ 访问邮箱历史，追踪变更

### 推送通知

- ❌ 设置或更新用户邮箱的推送通知监视（已禁用）
- ❌ 停止推送通知（已禁用）
...
使用 `list-messages`、`list-threads` 等工具的 **搜索查询** 参数时，可参考以下 Gmail 搜索操作符：

| 操作符 | 示例 | 说明 |
| ------ | ------ | ------ |
| `is:` | `is:unread` `is:starred` `is:important` | 按状态：未读、已加星标、重要等 |
| `from:` | `from:user@example.com` | 发件人 |
| `to:` | `to:team@company.com` | 收件人 |
| `subject:` | `subject:会议` | 主题包含关键词 |
| `has:attachment` | `has:attachment` | 带附件的邮件 |
| `filename:` | `filename:pdf` | 按附件类型或文件名 |
| `after:` / `before:` | `after:2024/1/1` `before:2024/12/31` | 按日期（yyyy/mm/dd） |
| `older_than:` / `newer_than:` | `older_than:7d` `newer_than:1m` | 按时间跨度（d=天/m=月/y=年） |
| `label:` | `label:INBOX` `label:重要` | 按标签 |
| `category:` | `category:primary` | 按分类（primary/social/promotions/updates/forums 等） |
| `size:` / `larger:` / `smaller:` | `larger:5M` | 按大小（K/M） |
| `OR` | `from:a OR from:b` | 满足任一条件（必须大写） |
| `-` | `-from:spam@x.com` | 排除某条件 |
| `" "` | `"exact phrase"` | 精确短语 |
