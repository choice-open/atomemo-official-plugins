...
export const createAssociationsTool = {
  name: "hubspot-create-associations",
  display_name: t("CREATE_ASSOCIATIONS_DISPLAY_NAME"),
  description: t("CREATE_ASSOCIATIONS_DESCRIPTION"),
  icon: "🔗",
  skill: "",
  parameters: [
    ...credentialParams,
  ...
  async invoke({ args }: { args: ToolArgs }): Promise<JsonValue> {
    ...
    try {
      const result = await client.crm.associations.v4.batchApi.createDefault(
        fromType,
        toType,
        {
          inputs: inputs.map((i) => ({
