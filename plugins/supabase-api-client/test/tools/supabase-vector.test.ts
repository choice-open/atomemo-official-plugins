...
const mockIndexScope = {
  ...
  listVectors: vi.fn().mockResolvedValue({
    data: { vectors: [], nextToken: undefined },
    error: null,
  }),
  queryVectors: vi
    .fn()
    .mockResolvedValue({ data: { matches: [] }, error: null }),
  deleteVectors: vi.fn().mockResolvedValue({ error: null }),
}

const mockBucketScope = {
  listIndexes: vi.fn().mockResolvedValue({
    data: { indexes: [], nextToken: undefined },
    error: null,
  }),
...
const mockVectors = {
  listBuckets: vi.fn().mockResolvedValue({
    data: { vectorBuckets: [], nextToken: undefined },
    error: null,
  }),
  createBucket: vi.fn().mockResolvedValue({ error: null }),
  getBucket: vi
    .fn()
    .mockResolvedValue({ data: { vectorBucket: {} }, error: null }),
...
describe("Vector tools – credential and shape", () => {
  ...
  describe("supabase-vector-list-buckets", () => {
    it("has correct name and params", () => {
      expect(supabaseVectorListBucketsTool.name).toBe(
        "supabase-vector-list-buckets",
      )
    ...
    it("calls listBuckets and returns success when credential provided", async () => {
      const r = await supabaseVectorListBucketsTool.invoke({
        args: {
          parameters: { supabase_credential: CRED_ID },
          credentials: cred,
        },
      })
  ...
  describe("supabase-vector-create-bucket", () => {
    it("has correct name and params", () => {
      expect(supabaseVectorCreateBucketTool.name).toBe(
        "supabase-vector-create-bucket",
      )
    ...
    it("calls createBucket and returns success", async () => {
      ...
      expect(r).toMatchObject({
        success: true,
        data: { vectorBucketName: "my-bucket" },
      })
  ...
  describe("supabase-vector-get-bucket", () => {
    it("has correct name and returns error when credential missing", async () => {
      expect(supabaseVectorGetBucketTool.name).toBe(
        "supabase-vector-get-bucket",
      )
  ...
  describe("supabase-vector-delete-bucket", () => {
    it("has correct name and returns error when credential missing", async () => {
      expect(supabaseVectorDeleteBucketTool.name).toBe(
        "supabase-vector-delete-bucket",
      )
  ...
  describe("supabase-vector-list-indexes", () => {
    it("has correct name and returns error when credential missing", async () => {
      expect(supabaseVectorListIndexesTool.name).toBe(
        "supabase-vector-list-indexes",
      )
  ...
  describe("supabase-vector-create-index", () => {
    it("has correct name and required params", () => {
      expect(supabaseVectorCreateIndexTool.name).toBe(
        "supabase-vector-create-index",
      )
    ...
    it("calls createIndex when credential and required params provided", async () => {
      ...
      expect(r).toMatchObject({
        success: true,
        data: { vectorBucketName: "b1", indexName: "idx1" },
      })
  ...
  describe("supabase-vector-get-index", () => {
    it("returns error when credential missing", async () => {
      const r = await supabaseVectorGetIndexTool.invoke({
        args: {
          parameters: { vector_bucket_name: "b1", index_name: "idx1" },
          credentials: {},
        },
      })
      expect(r).toEqual(missingCredError)
      ...
      const r = await supabaseVectorPutTool.invoke({
        args: {
          parameters: {
            vector_bucket_name: "b1",
            index_name: "idx1",
            vectors: '[{"key":"k1","data":{"float32":[0.1]}}]',
          },
          credentials: {},
        },
      })
  ...
  describe("supabase-vector-delete", () => {
    ...
    it("calls deleteVectors when credential and keys provided", async () => {
      ...
      expect(r).toMatchObject({ success: true })
      expect(mockIndexScope.deleteVectors).toHaveBeenCalledWith({
        keys: ["k1", "k2"],
      })
    })
  })
